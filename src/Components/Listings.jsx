...
function Listings() {
  ...
  const inputClass = "border border-[#334155] bg-[#0F172A] rounded-xl px-4 py-2.5 text-sm text-[#F8FAFC] outline-none focus:border-[#10B981] transition-all placeholder-[#475569] w-full";
      ...
      {isFarmer && (
        <div>
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-lg font-bold text-[#F8FAFC]">My Listings</h2>
              <p className="text-[#94A3B8] text-xs mt-0.5">{listings.length} products listed</p>
            </div>
            <button
          ...
          {/* Add product form */}
          {showForm && (
            <div className="bg-[#1E293B] border border-[#334155] rounded-2xl p-5 mb-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
              <h3 className="sm:col-span-2 text-sm font-bold text-[#F8FAFC] mb-1">New Product Details</h3>
              {["name", "category", "unit"].map((field) => (
                <div key={field} className="flex flex-col gap-1.5">
                  <label className="text-xs font-bold text-[#94A3B8] uppercase tracking-wider capitalize">{field}</label>
                  <input
                    name={field}
              ...
              <div className="flex flex-col gap-1.5 sm:col-span-2">
                <label className="text-xs font-bold text-[#94A3B8] uppercase tracking-wider">Product Image</label>
                <input
                  type="file"
                  accept="image/*"
                  name="image"
                  onChange={handleFile}
                  className="border border-[#334155] bg-[#0F172A] rounded-xl px-3 py-2 text-sm text-[#94A3B8] outline-none focus:border-[#10B981] file:mr-3 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-[#10B981] file:text-[#0F172A] cursor-pointer"
                />
              </div>
              <div className="flex flex-col gap-2 sm:col-span-2">
                <button
                  ...
                  className="w-full bg-[#10B981] hover:bg-[#059669] disabled:bg-[#334155] disabled:text-[#94A3B8] text-[#0F172A] font-bold py-3 rounded-xl transition-all flex items-center justify-center gap-2 shadow-md shadow-[#10B981]/20"
                >
                  {newProductLoading ? (
                    <>
          ...
          {/* Listings */}
          {loading ? (
            <div className="flex flex-col items-center gap-4 py-16 text-[#94A3B8]">
              <div className="w-8 h-8 border-2 border-[#334155] border-t-[#10B981] rounded-full animate-spin" />
            ...
            <div className="text-center py-16 bg-[#1E293B] border border-dashed border-[#334155] rounded-2xl">
              ...
              <p className="text-[#94A3B8] font-medium text-sm">No active listings yet.</p>
              <p className="text-[#475569] text-xs mt-1">Click "Add Product" to create your first listing!</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {listings.map((item) => (
                <div key={item.id} className="bg-[#1E293B] border border-[#334155] rounded-2xl p-4 flex flex-col gap-3 hover:border-[#10B981]/30 transition-all">
                  {item.image && (
                  ...
                  <p className="text-[#F59E0B] font-bold text-sm">
                    ₦{item.price.toLocaleString()} <span className="text-[#94A3B8] font-normal">/ {item.unit}</span>
                  </p>
